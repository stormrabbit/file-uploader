...
Defines the tooling that keeps production dependencies in sync between `file-uploader-desktop` and `file-uploader-server`. Because the desktop package bundles the server at build time, mismatched `dependencies` versions can cause subtle runtime failures; this capability enforces parity via an automated pre-build check.
